        ...
        @foreach(json_decode($messages,true) as $message)
            <div class="col-md-4">
                <div class="card card-msg">
                    <input type="hidden" value="{{$message['id']}}" class="msg-id">
                    <input type="hidden" value="{{$message['title']}}" class="msg-title">
                    <input type="hidden" value="{{$message['comment']}}" class="msg-comment">
                    <input type="hidden" value="{{$message['date_message']}}" class="msg-date">
                    <h5>Sujet :  {{$message['title']}}  </h5>
                    <h5>Date : {{$message['date_message']}}</h5>  
                    <h5>Message : {{ str_limit($message['comment'],10) }} </h5>
...
$(document).ready(function(){
     ...
     $(".card-msg").click(function(){
        ...
        var Message= $(this).children('.msg-comment').val();
         console.log(title);
         $.ajax({
             url:"{{route('formation_get_users','1')}}",
             beforeSend:function()
             {
                $(".loading").show();
             },
